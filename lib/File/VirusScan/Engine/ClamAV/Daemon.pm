package File::VirusScan::Engine::ClamAV::Daemon;
use strict;
use warnings;
use Carp;
...
sub new
{
	...
	if(exists $conf->{zip_fallback}) {
		unless (blessed($conf->{zip_fallback}) && $conf->{zip_fallback}->isa('File::VirusScan::Engine')) {
			croak q{The 'zip_fallback' config value must be an object inheriting from File::VirusScan::Engine};
		}
	}
...
File::VirusScan::Engine::ClamAV::Daemon - File::VirusScan backend for scanning with clamd
...
File::VirusScan::Engine::ClamAV::Daemon inherits from, and follows the
...
L<File::VirusScan::Engine> object that will be used as a fallback in the
