package Email::VirusScan::Engine::FPROT::DaemonV6;
use strict;
use warnings;
use Carp;
...
sub _scan_path
{
	...
	if(!$s->can_read($self->{read_timeout})) {
		$sock->close;
		return Email::VirusScan::Result->error("Timeout reading from fprot daemon");
	}

	my $resp = $sock->getline;
	if(!$resp) {
		$sock->close;
		return Email::VirusScan::Result->error("Did not get response from fprot while scanning $path");
	}

	my ($code, $desc, $name);
	unless (($code, $desc, $name) = $resp =~ /\A(\d+)\s<(.*?)>\s(.*)\Z/) {
		return Email::VirusScan::Result->error("Failed to parse response from fprotd: $path");
	}
	...
	#  7 128  At least one object was disinfected (clean now) (treat same as virus for Email::VirusScan)
	#
	...
	if($code & (1 | 128)) {
		...
		return Email::VirusScan::Result->error('FPROTD scanning interrupted by user');
	} elsif($code & 16) {
		return Email::VirusScan::Result->error('FPROTD platform error');
	} elsif($code & 32) {
		return Email::VirusScan::Result->error('FPROTD internal engine error');
	}
...
Email::VirusScan::Engine::FPROT::DaemonV6 - Email::VirusScan backend for scanning with F-PROT daemon, version 4
