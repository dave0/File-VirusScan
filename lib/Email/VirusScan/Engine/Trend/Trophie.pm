package Email::VirusScan::Engine::Trend::Trophie;
use strict;
use warnings;
use Carp;
...
sub scan_path
{
	...
	if(!$sock->print("$path\n")) {
		$sock->close;
		return Email::VirusScan::Result->error("Could not get trophie to scan $path");
	}
	...
	if(!$rc) {
		return Email::VirusScan::Result->error("Did not get response from trophie while scanning $path");
	}
	...
	return Email::VirusScan::Result->error('Unknown response from trophie');
}
...
Email::VirusScan::Engine::Trend::Trophie - Email::VirusScan backend for scanning with trophie
...
Email::VirusScan backend for scanning using the trophie daemon with
Trend's virus scanning libraries.

Email::VirusScan::Engine::Trend::Trophie inherits from, and follows the
...
from.  If you know, please contact the Email::VirusScan maintainers to
let us know.
